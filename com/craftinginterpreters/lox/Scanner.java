...
class Scanner {
    ...
    // Helper for matching characters
    ...
    // Helper for checking next character without advancing
    private char peek() {
        if (isAtEnd()) {
            return '\0';
        }
    ...
    // Helper for advancing in file
    private char advance() {
        current++;
        return source.charAt(current - 1);
    }

    // Default for adding token
    private void addToken(TokenType type, Object literal) {
        String text = source.substring(start, current);
        tokens.add(new Token(type, text, literal, line));
    }

    // Overload for adding token
